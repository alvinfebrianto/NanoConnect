...
export function MarqueeBrands() {
  return (
    <section className="relative w-full overflow-hidden border-zinc-200 border-y bg-white py-10 dark:border-zinc-800 dark:bg-zinc-950">
      <div className="flex overflow-hidden [mask-image:linear-gradient(to_right,transparent,black_128px,black_calc(100%-128px),transparent)]">
        <div className="flex min-w-full shrink-0 animate-marquee items-center justify-around gap-16 px-8">
          {NICHES.map(({ icon: Icon, label }) => (
            <div
              className="flex items-center gap-3 text-zinc-400 grayscale transition-all duration-300 hover:scale-110 hover:text-primary-600 hover:grayscale-0 dark:text-zinc-600 dark:hover:text-primary-500"
              key={label}
            >
