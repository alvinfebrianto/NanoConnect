...
export const createInfluencerMatchingService = () => {
  const calculateBudgetFit = (
    influencerPrice: number,
    totalBudget: number
  ): number => {
    const maxInfluencers = Math.floor(totalBudget / influencerPrice);
    if (maxInfluencers >= 3) {
      return 100;
    }
    if (maxInfluencers === 2) {
  ...
  const calculateLocationMatch = (
    ...
    const infLoc = influencerLocation.toLowerCase();
    const targetLoc = targetLocation.toLowerCase();

    if (infLoc === targetLoc) {
      return 100;
    }
    if (infLoc.includes(targetLoc) || targetLoc.includes(infLoc)) {
      return 80;
    }
    ...
    for (const region of sameRegion) {
      const inSameRegion =
        region.some((city) => infLoc.includes(city)) &&
        region.some((city) => targetLoc.includes(city));
      if (inSameRegion) {
        return 60;
      }
    }
  ...
  const calculateNicheMatch = (
    influencerNiche: string,
    campaignNiche: string
  ): number => {
    const nicheMapping: Record<string, string[]> = {
      "Kuliner & Makanan": ["food", "kuliner", "makanan", "restaurant"],
      "Fashion & Gaya Hidup": ["fashion", "lifestyle", "beauty", "kecantikan"],
      Teknologi: ["tech", "teknologi", "gadget", "review"],
      "Kecantikan & Perawatan Kulit": ["beauty", "skincare", "kecantikan"],
      "Travel & Petualangan": ["travel", "wisata", "adventure"],
      "Fitness & Kesehatan": ["fitness", "health", "gym", "wellness"],
      Gaming: ["gaming", "game", "esports"],
      "Bisnis & Keuangan": ["business", "finance", "bisnis", "keuangan"],
    };

    const infNiche = influencerNiche.toLowerCase();
    const campNiche = campaignNiche.toLowerCase();

    if (infNiche === campNiche) {
      return 100;
    }

    const keywords = nicheMapping[campaignNiche] ?? [];
    if (keywords.some((kw) => infNiche.includes(kw))) {
      return 85;
    }
  ...
  const calculateEngagementScore = (engagementRate: number): number => {
    if (engagementRate >= 6) {
      return 100;
    }
    if (engagementRate >= 4) {
  ...
  const calculatePreliminaryScore = (
    ...
    const nicheMatch = calculateNicheMatch(influencer.niche, campaign.niche);
