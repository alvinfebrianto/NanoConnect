  ...
  return (
    <section className="relative min-h-[80vh] w-full overflow-hidden bg-cream-50 pt-20 pb-16 lg:pt-32 lg:pb-20 dark:bg-zinc-950">
      <div className="absolute inset-0 z-0">
        ...
        <div className="absolute top-0 right-0 -mt-20 -mr-20 h-[500px] w-[500px] animate-pulse-slow rounded-full bg-accent-400/20 blur-[100px]" />
        <div className="absolute bottom-0 left-0 -mb-20 -ml-20 h-[500px] w-[500px] animate-pulse-slow rounded-full bg-accent-400/20 blur-[100px]" />
      </div>

      <div className="container relative z-10 mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col items-center text-center">
          <div className="mb-6 animate-fade-in opacity-0 [animation-delay:200ms] [animation-fill-mode:forwards]">
            <span className="inline-flex items-center gap-2 rounded-full bg-white/80 px-4 py-1.5 font-semibold text-accent-600 text-sm shadow-sm ring-1 ring-black/5 backdrop-blur-md transition-transform hover:scale-105 dark:bg-zinc-800/80 dark:ring-white/10">
          ...
          <h1 className="relative mb-6 max-w-4xl animate-slide-up font-bold font-display text-5xl text-zinc-900 leading-[0.9] tracking-tight opacity-0 [animation-delay:400ms] [animation-fill-mode:forwards] sm:text-6xl lg:text-8xl dark:text-zinc-50">
            Cerita Brand <br />
            <span className="relative z-10 inline-block bg-gradient-to-r from-accent-600 via-accent-500 to-accent-700 bg-clip-text text-transparent">
              Lebih Autentik
              <svg
                aria-hidden="true"
                className="absolute -bottom-2 left-0 -z-10 h-2 w-full text-accent-200"
          ...
          <div className="flex w-full animate-slide-up flex-col gap-4 opacity-0 [animation-delay:800ms] [animation-fill-mode:forwards] sm:w-auto sm:flex-row">
            <Link
              className="group relative inline-flex items-center justify-center overflow-hidden rounded-full bg-zinc-900 px-8 py-4 font-semibold text-lg text-white transition-all hover:scale-105 hover:bg-zinc-800 hover:shadow-accent-500/20 hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-zinc-900 focus:ring-offset-2"
              to="/influencers"
            >
              <span className="mr-2">Mulai Sekarang</span>
              <ArrowRight className="h-5 w-5 transition-transform group-hover:translate-x-1" />
              <div className="absolute inset-0 -z-10 bg-gradient-to-r from-accent-500 to-accent-600 opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
            </Link>
          </div>
        </div>
      </div>
      ...
      <div className="pointer-events-none absolute top-1/2 left-0 hidden -translate-y-1/2 animate-float opacity-0 [animation-delay:1000ms] [animation-fill-mode:forwards] lg:block">
        <div className="relative -left-12 h-40 w-40 rotate-12 rounded-3xl bg-white p-4 shadow-xl transition-transform hover:rotate-0 hover:scale-110">
          ...
          <div className="absolute -top-4 -right-4 rounded-full bg-accent-100 p-3 shadow-lg">
